...
import { ElSteps, ElStep, ElForm, ElFormItem, ElInput, ElRadioGroup, ElRadio, ElDatePicker, ElUpload, ElButton, ElMessage, ElCheckbox, ElIcon } from 'element-plus'
import { Plus } from '@element-plus/icons-vue'
import { LOST_TYPES } from '@/utils/constants'
        ...
        <ElForm ref="formRef" :model="form" label-width="100px" class="publish-form">
          <ElFormItem label="类型" prop="type" :rules="[{ required: true, message: '请选择类型' }]">
            <ElRadioGroup v-model="form.type">
              <ElRadio v-for="t in LOST_TYPES" :key="t" :value="t">{{ t }}</ElRadio>
            </ElRadioGroup>
          </ElFormItem>

          <ElFormItem label="物品名称" prop="name" :rules="[{ required: true, message: '请输入物品名称' }]">
            <ElInput v-model="form.name" placeholder="请输入物品名称" />
          </ElFormItem>

          <ElFormItem label="发生时间" prop="happenedAt" :rules="[{ required: true, message: '请选择时间' }]">
            <ElDatePicker v-model="form.happenedAt" type="datetime" placeholder="选择发生时间" style="width: 100%" />
          </ElFormItem>

          <ElFormItem label="地点" prop="location" :rules="[{ required: true, message: '请输入地点' }]">
            <ElInput v-model="form.location" placeholder="请输入发生地点" />
          </ElFormItem>

          <ElFormItem label="特征描述" prop="description" :rules="[{ required: true, message: '请描述特征' }]">
            <ElInput v-model="form.description" type="textarea" :rows="4" placeholder="请描述物品特征（颜色、品牌等）" />
          </ElFormItem>
      ...
      <template v-if="activeStep === 1">
        <div class="confirm-box">
          ...
          <div class="confirm-detail">
            ...
            <p><strong>物品名称：</strong>{{ form.name }}</p>
            <p><strong>发生时间：</strong>{{ form.happenedAt?.toLocaleString() || '未填' }}</p>
            <p><strong>地点：</strong>{{ form.location }}</p>
            <p><strong>特征描述：</strong>{{ form.description || '未填' }}</p>
